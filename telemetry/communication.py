...
class SimulatorBackend(QObject):
    """Backend simulado para desarrollar la interfaz sin conectar todavía el ESP32.

    La API está pensada para que más adelante pueda reemplazarse por un backend Wi-Fi
    manteniendo las mismas señales y métodos de control.
    """
    ...
    def set_mode(self, mode: str) -> None:
        ...
        if mode == self.MODE_TELEMETRY:
            # Nunca se entra a telemetría dejando actuadores comandados desde la PC.
            self.emergency_stop()
    ...
    def _tick(self) -> None:
        ...
        if self.mode == self.MODE_TEST:
            # En banco movemos lentamente la línea simulada para poder verificar
            # visualmente la respuesta de los 12 sensores.
            line_position = 5500 + 3000 * math.sin(elapsed * 0.65)
            sensors = self._sensor_values(line_position)
            self.sensors_updated.emit(sensors)
            return

        # Telemetría simulada de una vuelta. Estos coeficientes NO representan
        # todavía el PID real de Deimonyag; sirven únicamente para desarrollar la GUI.
        line_position = (
            5500
